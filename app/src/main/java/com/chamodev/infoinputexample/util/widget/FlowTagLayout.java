...
public class FlowTagLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
            ...
            if (shouldRelayout) {
                int newChildLeft = (int) ((float) (flowWidth - lastViewRight + firstViewLeft) / 2);
                for (View $childView : $childViewList) {
                    int $childWidth = $childView.getMeasuredWidth();

                    int $left = newChildLeft;
                    int $right = newChildLeft + $childWidth;

                    $childView.setLeft($left);
                    $childView.setRight($right);

                    newChildLeft += (mlp.leftMargin + $childWidth + mlp.rightMargin);
                }
            ...
            if (mAdapter instanceof OnInitSelectedPosition) {
                ...
                if (mTagCheckMode == FLOW_TAG_CHECKED_SINGLE) {
                    ...
                    if (isSelected) {
                        ...
                        TextView textView = childView.findViewById(R.id.flow_tag_tv);
